	...
	<script language='javascript'>
		...
		function feedPairedViewDialog(udata){
			getPairedParty(udata.user_id,udata.current_merge_id);
		}
		...
		function getPairedParty(user,mergeid){
			jQuery('#merged_loader').show();
			jQuery.ajax({
				url:'<?php echo base_url()."admin/users/getPairedParty/";?>'+user+'/'+mergeid,
				async:false,
				dataType: 'JSON',
				success:function(data){
					var i=0;
					//for(var i=0;i<2;i++){
						jQuery('#pp'+(i+1)+'user_id').html(data[i].user_id);
						jQuery('#pp'+(i+1)+'first_name').text(data[i].first_name);
					   jQuery('#pp'+(i+1)+'last_name').text(data[i].last_name);
					   jQuery('#pp'+(i+1)+'first_name').text(data[i].first_name);
					   jQuery('#pp'+(i+1)+'phone_number').text(data[i].phone_number);
					   jQuery('#pp'+(i+1)+'bank_name').text(data[i].bank_name);
					   jQuery('#pp'+(i+1)+'account_name').text(data[i].account_name);
					   jQuery('#pp'+(i+1)+'account_number').text(data[i].account_number);
					   jQuery('#pp'+(i+1)+'package').text(data[i].package);
					//}
					jQuery('#merged_loader').hide();
				},
				error:function(xhr,st,er){
		...
		function getPartiesForMerge(user, pack){
			...
			jQuery.ajax({
				url:'<?php echo base_url()."admin/users/getPartiesForMerge/";?>'+user+'/'+pack,
				async:false,
				dataType:'JSON',
				success:function(data){
					for(var i=0;i<2;i++){
		...
		function mergeParticipants(){jQuery('#merge_loader').show();
			jQuery.ajax({
				url: '<?php echo base_url()."admin/users/mergeParties/";?>'+jQuery('#muser_id').html()+'/'+jQuery('#p1user_id').html()+'/'+jQuery('#p2user_id').html()+'/'+jQuery('#mpackage').html(),
				async:false,
				success:function(data){
					alert('Message: '+data);
		...
		<!-- Model Dialog box for Confirm Automerge-->
		<div aria-hidden="false" aria-labelledby="confirmAutoMergeModal" role="dialog" tabindex="-1" class="modal hide fade in" id="confirmAutoMergeModal">
            <div class="modal-header">
              <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
              <h3 id="myModalLabel"> Confirm Auto Merge </h3>
            </div>
            <div class="modal-body">
              <h5>Are you sure you want to continue <em>Auto Merge</em> of users in this package?</h5>
			  <!--<p>Account Information:</p>
			  <p>Name: <span id="cfirst_name"></span> <span id="clast_name"></span></p>-->
			  <p>Package: <?=$package?></p>
			  <p>Total Qualified Users: <?=count($users_by_package)?></p>
            </div>
            <div class="modal-footer">
              <a id="cam" href="<?php echo base_url()?>admin/users/autoMerge/<?=$package.'/'.$status?>" class="btn btn-primary" >Ok</a>
			  <button data-dismiss="modal" class="btn">No</button>
            </div>
		</div><!--#confirmAutoMergeModal-->
		<!-- Model Dialog box for Confirm UnBlock-->
		<div aria-hidden="false" aria-labelledby="confirmBlockModal" role="dialog" tabindex="-1" class="modal hide fade in" id="confirmBlockModal">
            <div class="modal-header">
              <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
              <h3 id="myModalLabel"> Confirm UnBlock </h3>
            </div>
            <div class="modal-body">
		...
		<div aria-hidden="false" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" class="modal hide fade in" id="mergeModal">
            ...
            <tr>
			  ...
			  <div>
			  <table width="100%"><tr>
				<div id="participants_loader" class="btn-warning">Loading participants...</div>
				<td>
		...
		<!-- Model Dialog box for fetching merged parties-->
		...
		<!-- Model Dialog box for fetch Paired Party-->
		<div aria-hidden="false" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" class="modal hide fade in" id="pairedDetailsModal">
            <div class="modal-header">
              <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
              <h3 id="myModalLabel">Merge Detail</h3>
            </div>
            <div class="modal-body">
				<div id="merged_loader" class="btn-warning hide">Fetching merged parties...please wait.</div>
              <h4>Paired Party</h4><div class="hide" id="pp1user_id"></div>
					<p>Full Name: <span id='pp1first_name'></span> <span id='pp1last_name'></span></p>
					<p>Phone Number: <span id='pp1phone_number'></span></p>
					<p>Account Name: <span id='pp1account_name'></span></p>
					<p>Account Number: <span id='pp1account_number'></span></p>
					<p>Bank Name: <span id='pp1bank_name'></span></p>
					<p>Package: <span id='pp1package'></span></p>
            </div>
            <div class="modal-footer">
              <!--<button data-dismiss="modal" class="btn">Decline</button>
              <button class="btn btn-primary">Accept</button>-->
